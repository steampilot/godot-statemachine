...
func mirror_intent(original_player: Node2D) -> void:
    ...
    _mirrored_intent = {
        "facing_direction": original_player.get_meta("facing_direction",
        	1) if original_player.has_meta("facing_direction") else 1,
        "velocity": original_player.velocity if "velocity" in original_player else Vector2.ZERO,
        "animation_state": original_player.get_meta("animation_state",
        	"idle") if original_player.has_meta("animation_state") else "idle",
        "is_jumping": original_player.get_meta("is_jumping",
        	false) if original_player.has_meta("is_jumping") else false,
    }
